...
const ShopContextProvider = (props) => {
    ...
    const onSelectSize = (item, size) => { handleSizeSelect({ id: item.id, name: item.name, image: item.image, old_price: item.old_price, new_price: item.new_price }, size) }
    ...
    const removeFromCart = (item) => {
        setWhatIsInCart(prevWhatIsInCart => {
            ...
            if (index !== -1) {
                if (updatedCartItems[index].quantity > 1) { updatedCartItems[index].quantity -= 1 } else {
